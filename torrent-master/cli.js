  ...
  getSource(source, function (body) {
    ...
    dl.on('ready', function () {
      ...
      var status = function () {
        ...
        if (percentage >= 100) { 
          percentage = 100
          var names = dl.files.map(function (f) { return f.name })
          var stringified = '{"names" : '.concat(JSON.stringify(names), '}')
          var path = argv.path
          try{
          ...
          path = path.concat('/', 'tranfer-info.json')
          try {
...
function failed() {
  var path = argv.path.replaceAll('\\', '/').concat('/', 'tranfer-info.json')
  try {
    fs.writeFileSync(path, '{"names" : ["DownloadErrorExecption"]}');
    // fichier écrit avec succès
  } catch (err) {
    console.error(err);
  }
